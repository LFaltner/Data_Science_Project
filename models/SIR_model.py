...
class SIR_model():
    ...
    def create_scenario(self, name, scenario_end_list, rho_constant_list=None, sigma_constant_list=None, plot=False):
        ...
        for i, (phase_date, rho_constant, sigma_constant) in enumerate(
                zip(scenario_end_list, rho_constant_list, sigma_constant_list)):
        ...
        if plot:
            ...
            mask = np.array([(pd.to_datetime(self.actual_df.index) >= self.z.index[0]) & (
                        pd.to_datetime(self.actual_df.index) <= self.z.index[-1])]).reshape(-1)
            fig, ax = plt.subplots()
